...
async function diagnose() {
  ...
  try {
    ...
    if (!tableNames.includes("user")) {
      console.error(
        '   ❌ Tabel "user" tidak ditemukan! Jalankan SQL migrations dulu.',
      );
      return;
    }
  } catch (err) {
  ...
  try {
    const [cols] = await pool.query(`SHOW COLUMNS FROM user`);
  ...
  try {
    ...
    if (users.length === 0) {
      ...
      users.forEach((u, i) => {
        ...
        console.log(
          `        role    : ${u.nama_role || "(null)"} (id_role=${u.id_role})`,
        );
        console.log(`        disabled: ${u.disabled}`);
        console.log(
          `        hash    : ${u.hash_prefix}... (panjang: ${u.hash_length})`,
        );
        const isValidBcrypt =
          u.hash_prefix === "$2b$10$" ||
          u.hash_prefix === "$2a$10$" ||
          u.hash_prefix?.startsWith("$2");
        console.log(
          `        bcrypt? : ${isValidBcrypt ? "✅ Ya (format benar)" : "❌ BUKAN bcrypt! Password belum di-hash!"}`,
        );
        console.log("");
      });
    }
  ...
  try {
    ...
    console.log(
      `   ${total > 0 ? "✅" : "⚠️"} role_permission: ${total} baris\n`,
    );
  } catch (err) {
        ...
        res.on("end", () => {
          ...
          try {
            ...
            if (res.statusCode === 200) {
              console.log(
                `   ✅ Login BERHASIL dengan user '${testUser.username}'`,
              );
              ...
              if (json.message === "Pengguna tidak ditemukan") {
                console.log(
                  '   ➡  User tidak ada di database atau "disabled = 1"',
                );
              } else if (json.message === "Password salah") {
                console.log(
                  "   ➡  Password tidak cocok. Pastikan password di-hash dengan bcrypt",
                );
              }
            }
          } catch {
