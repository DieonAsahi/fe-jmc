            ...
            <div class="row g-4">
              <div class="col-12">
                <div class="row align-items-center">
                  <div class="col-auto">
                    <img
                      :src="previewFoto || '/favicon.png'"
                      alt=""
                      class="foto-ptofil"
                    />
                    <label
                      for="unggah-foto"
                      class="form-label text-primary text-center cursor-pointer d-block"
                      >Ubah Foto</label
                    >
                    <input
                      id="unggah-foto"
              ...
              <div class="col-md-4" style="position: relative">
                ...
                <ul
                  v-if="
                    kecamatanSuggestions.length > 0 && kecamatanDropdownOpen
                  "
              ...
              <div class="col-md-4">
                ...
                <input
                  v-model="form.jarak_rumah_kantor"
                  type="number"
                  min="0"
                  step="0.5"
                  class="form-control"
...
async function onKecamatanSearch() {
  ...
  kecamatanDebounce = setTimeout(async () => {
    try {
      const res = await get(
        `/master/wilayah?search=${encodeURIComponent(val)}`,
      );
...
async function handleSubmit() {
  ...
  try {
    ...
    const payload = {
      ...
      jarak_rumah_kantor: form.jarak_rumah_kantor
        ? Number(form.jarak_rumah_kantor)
        : null,
      pendidikan: form.pendidikan.filter(
        (p) => p.tingkat_pendidikan || p.nama_sekolah,
      ),
    };
...
onMounted(async () => {
  ...
  if (props.id) {
    try {
      const res = await get(`/pegawai/${props.id}`);
      const d = res.data;
      Object.assign(form, {
        nip: d.nip || "",
      ...
      if (d.foto_pegawai)
        previewFoto.value = `/images/pegawai/${d.foto_pegawai}`;
