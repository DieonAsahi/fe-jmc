        ...
        <div class="modal-body">
          ...
          <div class="mb-3">
            ...
            <input
              ...
              class="form-control"
              minlength="6"
              @keyup="validasiUsername"
            />
            <small class="text-secondary"
              >Min 6 karakter, huruf kecil & angka</small
            >
          </div>
...
async function onPegawaiSearch() {
  ...
  pegawaiDebounce = setTimeout(async () => {
    try {
      const res = await get(
        `/pegawai?search=${encodeURIComponent(val)}&limit=10`,
      );
...
async function simpanUser() {
  if (!form.id_pegawai)
    return alert("Silakan pilih Nama Pengguna dari daftar autosuggest!");
  if (!form.username || form.username.length < 6)
    return alert("Username minimal 6 karakter");
  ...
  try {
    ...
    payload.id_departemen = payload.id_departemen
      ? Number(payload.id_departemen)
      : null;
