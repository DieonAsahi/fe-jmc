...
async function hitungTunjangan() {
  ...
  try {
    ...
    const res = await post("/tunjangan/transport/hitung", {
      tahun: Number(tahun),
      bulan: Number(bulan),
    });
    ...
    <div class="card">
      <div class="card-header d-flex gap-2 align-items-center">
        ...
        <span
          ...
          class="ms-2 small"
          :class="
            hitungMessage.includes('berhasil') ? 'text-success' : 'text-danger'
          "
        >
          {{ hitungMessage }}
        </span>
      ...
      <template
        v-if="
          detailData && detailData.penerima && detailData.penerima.length > 0
        "
      >
      ...
      <div v-else class="card-body text-center py-4">
        Belum ada data. Klik <strong>"Hitung Tunjangan"</strong> untuk
        menghitung.
      </div>
    </div>
  </div>
