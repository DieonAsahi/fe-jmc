...
const MODULE_MAP: Array<{ pattern: string; module: string | null }> = [
  ...
  {
    pattern: "tunjangan/transport/hitung",
    module: "modul_tunjangan_transport",
  },
...
export async function checkPermission(
  ...
  if (!perm.akses)
    return { allowed: false, reason: "Tidak memiliki akses ke modul ini" };

  if (action === "create") {
    if (!perm.create)
      return { allowed: false, reason: "Tidak memiliki akses Create" };
  } else if (perm[action] === "No") {
    return {
      allowed: false,
      reason: `Tidak memiliki akses ${action.charAt(0).toUpperCase() + action.slice(1)}`,
    };
  }
